...
describe.each([originateFixedPriceAdminSale])(
    ...
    originateMarketplace => {
        ...
        async function mintNftTokens(
            ...
            $log.info(
                `Minted non-fungible tokens. Consumed gas: ${op.consumedGas}`
            );
        }
        ...
        async function mintFtTokens(
            ...
            const hash = await op.confirmation();
            $log.info(
                `Minted fungible tokens. Consumed gas: ${op.consumedGas}`
            );
        }
        ...
        async function createFtToken(
            ...
            const hash = await op.confirmation();
            $log.info(
                `Created fungible token. Consumed gas: ${op.consumedGas}`
            );
        }
        ...
        test('bob makes sale, and alice buys nft', async () => {
            ...
            await createFtToken(tezos.alice, {
                token_id: ftTokenId,
            ...
            const [
                aliceHasNFTTokenBefore,
                bobHasNFTTokenBefore
            ] = await hasNftTokens([
            ...
            const pauseOp = await marketplace.methods
                .pause(true)
                .send({ amount: 0 });
            $log.info(`Waiting for ${pauseOp.hash} to be confirmed...`);
            const pauseOpHash = await pauseOp
                .confirmation()
                .then(() => pauseOp.hash);
            ...
            try {
                ...
                $log.info(
                    `Confirmation: Cannot create sale while contract is paused`
                );
            }

            $log.info('unpause marketplace');
            const unpauseOp = await marketplace.methods
                .pause(false)
                .send({ amount: 0 });
            $log.info(`Waiting for ${unpauseOp.hash} to be confirmed...`);
            const unpauseOpHash = await unpauseOp
                .confirmation()
                .then(() => unpauseOp.hash);
            ...
            const sellOpHash = await sellOp
                .confirmation()
                .then(() => sellOp.hash);
            $log.info(`Operation injected at hash=${sellOpHash}`);

            const aliceSaleContract = await tezos.alice.contract.at(
                marketplace.address
            );
            ...
            const buyOp = await aliceSaleContract.methods
                ...
                .send({ source: aliceAddress, amount: 0 });
        ...
        test('bob makes sale, cancels it, then alice unsuccessfully tries to buy', async () => {
            ...
            const removeSaleOpHash = await removeSaleOp
                .confirmation()
                .then(() => removeSaleOp.hash);
            ...
            try {
                $log.info(
                    `Alice buys non-fungible token with her fungible tokens`
                );
                const buyOp = await aliceSaleContract.methods
                    .buy(
                        bobAddress,
