...
import Lorentz.Contracts.Spec.FA2Interface (TokenId(TokenId), mkFA2View)
...
hprop_Every_sale_sends_a_fee_to_the_fee_collector =
  property $ do
    ...
    clevelandProp $ do
      ...
      feeCollectorsBalance <- balanceOf moneyFA2 consumer moneyTokenId feeCollector
...
hprop_Tokens_are_transferred_to_seller  =
  property $ do
    ...
    clevelandProp $ do
      ...
      sellerBalance <- balanceOf moneyFA2 consumer moneyTokenId seller
...
hprop_NFT_is_transferred_to_buyer  =
  property $ do
    ...
    clevelandProp $ do
      ...
      sellerNftBalance <- balanceOf nftFA2 consumer nftTokenId seller
      sellerNftBalance @== 0
      buyerNftBalance <- balanceOf nftFA2 consumer nftTokenId buyer
      buyerNftBalance @== 1
...
hprop_Contracts_balance_is_zero_after_a_sale_is_concluded =
  property $ do
    ...
    clevelandProp $ do
      ...
      contractTokenBalance <- balanceOf moneyFA2 consumer moneyTokenId contract
      contractNftBalance <- balanceOf nftFA2 consumer nftTokenId contract
  ...
  property $ do
    ...
    clevelandProp $ do
      ...
      initialGlobalBalance <- sum <$> forM [feeCollector, buyer, seller] \account ->
        balanceOf moneyFA2 consumer moneyTokenId account
...
balanceOf
  ...
  => TAddress FA2.FA2SampleParameter -> TAddress [FA2.BalanceResponseItem] -> TokenId -> addr -> m Natural
balanceOf fa2 consumer tokenId account = do
  call fa2 (Call @"Balance_of") (mkFA2View [FA2.BalanceRequestItem (toAddress account) tokenId] consumer)
  consumerStorage <- fromVal @[[FA2.BalanceResponseItem]] <$> getStorage consumer
  pure $ FA2.briBalance $ Unsafe.head $ Unsafe.head consumerStorage
