...
genTestData = do
  let genNat = Gen.integral (Range.constant 0 20)
  numOffers <- genNat
  token1Offer <- genNat
  token2Offer <- genNat 
...
hprop_Correct_final_balances_on_acceptance = 
  property $ do
   TestData{token1Offer, token2Offer, token1Request, token2Request} <- forAll genTestData 
   clevelandProp  $ do
      setup <- doFA2Setup
      ...
      assertingBalanceDeltas fa2
        [ (alice, tokenId1) -: negateInteger (fromIntegral $ token1Offer)
        , (alice, tokenId2) -: negateInteger (fromIntegral $ token2Offer)
