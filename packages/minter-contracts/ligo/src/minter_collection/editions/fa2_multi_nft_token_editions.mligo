...
type editions_entrypoints = 
 ...
 | Mint_editions of mint_collection list
...
let mint_editions ( collection_list , storage : mint_collection list * editions_storage)
  : operation list * editions_storage =
  let mint_single_collection : (editions_storage * mint_collection) -> editions_storage = fun (storage, param : editions_storage * mint_collection) ->
    let u : unit = assert_msg(param.number_of_editions <= storage.max_editions_per_collection,
       "COLLECTION_TOO_LARGE" ) in
  ...
  let new_storage = List.fold mint_single_collection collection_list storage in
...
let distribute_edition_to_addresses ( edition_id, token_info, receivers, edition_metadata, storage : nat *  ((string, bytes) map) * (address list) * edition_metadata * editions_storage)
  ...
  let intial_token_id : nat = (edition_id * storage.max_editions_per_collection) + abs (edition_metadata.number_of_editions - edition_metadata.number_of_editions_to_distribute) in 
...
let sample_storage : editions_storage = {
  ...
  max_editions_per_collection = 1000n;
  nft_asset_storage = {
    admin  = {
