...
let resolve_auction(asset_id, storage : nat * storage) : return = begin
    ...
    let oplist = 
      if first_bid(auction) or auction.offchain_bid
      then fa2_transfers 
      else
...
let place_bid(bid_data, storage : bid_data * storage) : return = begin
    let auction : auction = get_auction_data(bid_data.asset_id, storage) in
    ...
    let op_list : operation list = 
      if first_bid(auction) or auction.offchain_bid then
        ([] : operation list) else
        let return_bid : operation = transfer_tez(auction.current_bid,  auction.highest_bidder) in
        [return_bid] in
    ...
    let updated_auction_data = {auction with current_bid = bid_amount; highest_bidder = Tezos.sender; 
                                last_bid_time = Tezos.now; end_time = new_end_time; offchain_bid = bid_data.offchain_bid} in
    let updated_auctions = Big_map.update bid_data.asset_id (Some updated_auction_data) storage.auctions in
    (op_list , {storage with auctions = updated_auctions})
  end
...
let english_auction_tez_no_configure (p,storage : auction_without_configure_entrypoints * storage) : return =
  match p with
    | Bid bid_data -> place_bid(bid_data, storage)
